...
export class ConfigManager extends EventEmitter {
    ...
    private validateConfig(config: ServerConfig): void {
        ...
        // 并发数验证（放宽至 1-300）
        if (!Number.isInteger(config.utls.concurrency) || config.utls.concurrency < 1 || config.utls.concurrency > 300) {
            throw new Error(`并发数无效: ${config.utls.concurrency}（必须在 1-300 之间）`)
        }
        ...
        // uTLS 连接复用与自适应并发配置校验（如存在则校验）
        if (config.utls.enableKeepAlive !== undefined && typeof config.utls.enableKeepAlive !== 'boolean') {
            throw new Error(`utls.enableKeepAlive 无效（必须为 boolean）`)
        }
        if (config.utls.enableAdaptiveConcurrency !== undefined && typeof config.utls.enableAdaptiveConcurrency !== 'boolean') {
            throw new Error(`utls.enableAdaptiveConcurrency 无效（必须为 boolean）`)
        }
        if (config.utls.adaptiveConcurrency) {
            const ac = config.utls.adaptiveConcurrency
            if (!Number.isInteger(ac.adjustmentInterval) || ac.adjustmentInterval < 1000 || ac.adjustmentInterval > 10 * 60 * 1000) {
                throw new Error(`utls.adaptiveConcurrency.adjustmentInterval 无效: ${ac.adjustmentInterval}（1s-10m）`)
            }
            if (!Number.isInteger(ac.minConcurrency) || ac.minConcurrency < 1 || ac.minConcurrency > 300) {
                throw new Error(`utls.adaptiveConcurrency.minConcurrency 无效: ${ac.minConcurrency}（1-300）`)
            }
            if (!Number.isInteger(ac.maxConcurrency) || ac.maxConcurrency < 1 || ac.maxConcurrency > 300) {
                throw new Error(`utls.adaptiveConcurrency.maxConcurrency 无效: ${ac.maxConcurrency}（1-300）`)
            }
            if (ac.maxConcurrency < ac.minConcurrency) {
                throw new Error(`utls.adaptiveConcurrency.maxConcurrency 必须 >= minConcurrency`)
            }
            if (!Number.isFinite(ac.responseTimeThreshold) || ac.responseTimeThreshold < 100 || ac.responseTimeThreshold > 10000) {
                throw new Error(`utls.adaptiveConcurrency.responseTimeThreshold 无效: ${ac.responseTimeThreshold}（100-10000ms）`)
            }
            if (!Number.isFinite(ac.successRateThreshold) || ac.successRateThreshold < 0 || ac.successRateThreshold > 1) {
                throw new Error(`utls.adaptiveConcurrency.successRateThreshold 无效: ${ac.successRateThreshold}（0-1）`)
            }
        }
        ...
        if (anyConfig.dataValidation && anyConfig.dataValidation.minResponseSize !== undefined) {
        ...
        // DNS 配置（可选）存在则验证
        if (anyConfig.dns) {
            const dns = anyConfig.dns
            if (dns.ipPoolUsageRate !== undefined) {
                if (typeof dns.ipPoolUsageRate !== 'number' || dns.ipPoolUsageRate < 0 || dns.ipPoolUsageRate > 1) {
                    throw new Error(`dns.ipPoolUsageRate 无效: ${dns.ipPoolUsageRate}（必须在 0-1 之间）`)
                }
            }
            if (dns.health) {
                const dh = dns.health
                if (!Number.isInteger(dh.consecutiveFailsThreshold) || dh.consecutiveFailsThreshold < 1 || dh.consecutiveFailsThreshold > 100) {
                    throw new Error(`dns.health.consecutiveFailsThreshold 无效: ${dh.consecutiveFailsThreshold}（必须在 1-100 之间）`)
                }
                if (dh.successRateThreshold < 0 || dh.successRateThreshold > 1) {
                    throw new Error(`dns.health.successRateThreshold 无效: ${dh.successRateThreshold}（必须在 0-1 之间）`)
                }
                if (!Number.isInteger(dh.blacklistDuration) || dh.blacklistDuration < 1000 || dh.blacklistDuration > 24 * 60 * 60 * 1000) {
                    throw new Error(`dns.health.blacklistDuration 无效: ${dh.blacklistDuration}（1秒-24小时，单位ms）`)
                }
                if (!Number.isInteger(dh.minPoolSize) || dh.minPoolSize < 0 || dh.minPoolSize > 10000) {
                    throw new Error(`dns.health.minPoolSize 无效: ${dh.minPoolSize}（必须在 0-10000 之间）`)
                }
            }
        }
